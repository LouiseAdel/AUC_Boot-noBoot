...
def compute_auc_and_pvalues(y_true, y_scores, baseline_values=[0.5, 0.9636, 1.0]):
    # Compute the AUC
    auc = roc_auc_score(y_true, y_scores)
    
    # Compute the standard error of the AUC (estimated via the formula for standard error of AUC)
    n = len(y_true)
    # Calculate standard error for AUC (this is a simplification)
...
auc_1, se_1, pvals_1 = compute_auc_and_pvalues(y_true_1, y_scores_1, baseline_values)
auc_2, se_2, pvals_2 = compute_auc_and_pvalues(y_true_2, y_scores_2, baseline_values)
auc_3, se_3, pvals_3 = compute_auc_and_pvalues(y_true_3, y_scores_3, baseline_values)
